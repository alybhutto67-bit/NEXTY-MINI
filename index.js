                            ...
                            (async () => {
                                try {
                                    ...
                                    switch (commandName) {
                                        ...
                                        case 'settings': case 'setting': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, args, this); break;
                                        ...
                                        case 'antidelete': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['antidelete', ...args], this); break;
                                        case 'antiedit': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['antiedit', ...args], this); break;
                                        case 'antistatus': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['antistatus', ...args], this); break;
                                        case 'antibug': await commands.antibug(this.sock, from, msg, isOwner, botData, saveBotData, args); break;

                                        // ===== STATUS / AUTO =====
                                        case 'status': 
                                        case 'autostatus': await commands.autostatus(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, args); break;
                                        case 'autoreacts': await commands.autoreacts(this.sock, from, msg, isOwner, this, args); break;
                                        case 'autoread': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['autoread', ...args], this); break;
                                        case 'autoview': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['autoview', ...args], this); break;
                                        case 'autolike': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['autolike', ...args], this); break;
                                        case 'autodownload': await commands.settings(this.sock, from, msg, isOwner, botData, saveBotData, this.userId, ['autodownload', ...args], this); break;
...
break;
            ...
            this.sock.ev.on('messages.update', async (updates) => {
                for (const update of updates || []) {
                    await handleEditedMessage(this.sock, update, messageLogs);
                }
            });
...
io.on('connection', (socket) => {
    ...
    socket.on('pair-request', async ({ userId, number }) => {
        try {
        if (!userId || !number) {
            socket.emit('pair-error', 'Phone number or session ID is missing. Please try again.');
            return;
        }
        if (sessions[userId]) {
            if (!botData.statusSettings[userId]) {
            ...
            console.error(`[pair-request] ${userId || 'unknown'}:`, error.message);
            socket.emit('pair-error', `Pairing failed: ${error.message || 'server could not initialize the session'}`);
        }
    });
